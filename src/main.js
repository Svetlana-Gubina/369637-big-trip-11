...
const getRouteInfoElement = (dep, arr, start, point, end, total) => {
...
const filtersArr = [`Everything`, `Future`, `Past`];
for (let filter of filtersArr) {
  render(filtersForm, getFilter(filter), `afterbegin`);
}

const tripEvents = document.querySelector(`.trip-events`);
render(tripEvents, getSort(), `afterbegin`);
render(tripEvents, getForm(), `beforeend`);
